...
export const Container = (props) =>
  {
    ...
    const allFieldsInModal = async () => {
      const response = await getApiWithAuth(
        `${Url.ViewClubHiddenAttribute}${id}`
      );
      if (response.success) {
    ...
    return (
      <div style={{}}>
        {isLoading ? (
          ...
          <div style={style} className="ticketCardStyle">
            <div className="cardScrollContent">
              <Row gutter={[0,16]}>
                {cards.length == 0 ? (
                  ...
                  cards.length > 0 &&
                  cards.filter( (item) => item !== null && item.key !== "idstatus" ).map((card, index) => {
                    return (
                      <>
                        <Col xxl={12} xl={24}>
                          <Card
